...
class ScheduleFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        (activity as FragmentActivity).getSchedule() //node서버에서 값 받아오는 함수
        ...
        val color: Array<Int> = //배경 color
                arrayOf(Color.rgb(230, 230, 250),
        ...
        for (i in 0 until dayList.size) {
            var a: Int = SharedPreferenceManager.getInt(mContext, "PREFDAY") // 이전 인덱스의 값 가져오기
            var b: Int = SharedPreferenceManager.getInt(mContext, "PREFTIME")
            ...
            if (a == dayList[i].toInt() && b == timeList[i].toInt()) {
